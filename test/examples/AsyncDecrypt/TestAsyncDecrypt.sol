...
abstract contract TestAsyncDecrypt is GatewayCaller {
    ...
    function requestBool(uint256 msgValue) public {
        uint256[] memory cts = new uint256[](1);
        cts[0] = Gateway.toUint256(xBool);
        /// @dev Request decryption with a 100-second deadline and non-trustless mode
        Gateway.requestDecryption(cts, this.callbackBool.selector, msgValue, block.timestamp + 100, false);
    }
    ...
    /// @notice Request decryption of an 8-bit unsigned integer
    function requestUint8Trustless() public {
        uint256[] memory cts = new uint256[](1);
        cts[0] = Gateway.toUint256(xUint8);
        Gateway.requestDecryption(cts, this.callbackUint8Trustless.selector, 0, block.timestamp + 100, true);
    }

    /// @notice Callback function for 8-bit unsigned integer decryption
    /// @param decryptedInput The decrypted 8-bit unsigned integer
    /// @return The decrypted value
    function callbackUint8Trustless(uint256, uint8 decryptedInput, bytes[] memory /*signatures*/ )
        public
        onlyGateway
        returns (uint8)
    {

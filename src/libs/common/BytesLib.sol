...
library BytesLib {
    function padLeft(bytes memory b, uint256 len) internal pure returns (bytes memory) {
        require(b.length <= len, "padLeft, buffer too large");
        if (b.length == len) {
            return b;
        }
        bytes memory zeros = new bytes(len - b.length);
        return bytes.concat(zeros, b);
    }

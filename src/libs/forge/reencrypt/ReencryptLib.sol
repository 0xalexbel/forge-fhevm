...
library ReencryptLib {
    ...
    function generateKeyPair(IRandomGenerator randomGenerator)
        internal
        returns (bytes memory publicKey, bytes memory privateKey)
    {
        require(address(randomGenerator) != address(0), "Null random generator address");
        IVmSafe.Wallet memory w = vm.createWallet(randomGenerator.randomUint());

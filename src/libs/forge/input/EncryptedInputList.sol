...
library EncryptedInputListLib {
    ...
    function addBytes128(EncryptedInputList memory self, bytes memory value, bytes32 random) internal pure {
        ...
        EncryptedInputItem memory e;
        e._type = TFHEHandle.ebytes128_t;
        e._numBits = 1024;
        e._data = bytes.concat(bytes1(e._type), pad, value, random);
        e._bytesValue = value;
        _addItem(self, e);
    ...
    function addBytes256(EncryptedInputList memory self, bytes memory value, bytes32 random) internal pure {
        ...
        EncryptedInputItem memory e;
        e._type = TFHEHandle.ebytes256_t;
        e._numBits = 2048;
        e._data = bytes.concat(bytes1(e._type), pad, value, random);
        e._bytesValue = value;
        _addItem(self, e);
    ...
    function addBytes(EncryptedInputList memory self, bytes memory value, bytes32 random, uint16 numBytes, uint8 typeCt)
        internal
        pure
    {
        require(value.length <= numBytes);
        require(
            ((typeCt == TFHEHandle.ebytes256_t) && numBytes == 256)
                || ((typeCt == TFHEHandle.ebytes128_t) && numBytes == 128)
                || ((typeCt == TFHEHandle.ebytes64_t) && numBytes == 64)
        );
        bytes memory pad = new bytes(numBytes - value.length);
        EncryptedInputItem memory e;
        e._type = typeCt;
        e._numBits = numBytes * 8;
        e._data = bytes.concat(bytes1(e._type), pad, value, random);
        e._bytesValue = value;
        _addItem(self, e);
    }
    ...
    function encrypt(
        ...
        if (signer.coprocSigner.privateKey != 0) {

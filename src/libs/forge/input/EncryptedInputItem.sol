...
library EncryptedInputItemLib {
    function handle(EncryptedInputItem memory self, bytes32 hashedData) internal pure returns (bytes32) {
    ...
    function is256(EncryptedInputItem memory self) internal pure returns (bool) {
        //return (self._type <= Common.euint256_t);
        return TFHEHandle.isType256Bits(self._type);
    }

    function is2048(EncryptedInputItem memory self) internal pure returns (bool) {
        return !is256(self);
    }

    function extract256(EncryptedInputItem memory self) internal pure returns (uint256) {
    ...
    function extract2048(EncryptedInputItem memory self) internal pure returns (uint256[8] memory b2048) {

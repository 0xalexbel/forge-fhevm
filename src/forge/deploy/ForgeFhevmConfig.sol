...
import {USE_DETERMINISTIC_RANDOM_GENERATOR, PRIVATE_KEY_FHEVM_DEBUGGER_DEPLOYER} from "../envDebugger.default.sol";

struct ForgeFhevmConfig {
    string mnemonic;
    ...
    AddressLib.Signer fhevmDebuggerDeployer;
    AddressLib.Signer randomGeneratorDeployer;
}

library ForgeFhevmConfigLib {
    IVmSafe private constant vm = IVmSafe(forgeStdVmSafeAdd);

    function initializeWithEnv(ForgeFhevmConfig memory self) internal view {
        ...
        self.fhevmDebuggerDeployer = EnvLib.envSigner("PRIVATE_KEY_FHEVM_DEBUGGER_DEPLOYER", PRIVATE_KEY_FHEVM_DEBUGGER_DEPLOYER);
    }

    function storageCopyFrom(ForgeFhevmConfig storage self, ForgeFhevmConfig memory src) internal {
        ...
        self.fhevmDebuggerDeployer = src.fhevmDebuggerDeployer;
    }

    function getKmsSignersPk(ForgeFhevmConfig memory self) internal pure returns (uint256[] memory) {
    ...
    function getKmsSignersAddr(ForgeFhevmConfig memory self) internal pure returns (address[] memory) {
...
using ForgeFhevmConfigLib for ForgeFhevmConfig global;
